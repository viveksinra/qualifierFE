...
export default function Blog({ match }) {
	...
	return (
		<Fragment>
			...
			<div className={classes.blogTopBg}>Blogs & News</div>
				...
				<Grid container spacing={2}>
					<Grid item xs={12} md={9}>
						<Paper className={classes.paper}>
							...
							<Suspense fallback={<Skeleton width="400" variant="rect" />}>
								<BlogCard page={page} newData={newData} setNewD={(d) => setNewD(d)} />
							</Suspense>
							{loading ? (
...
const BlogCard = ({ page, newData, setNewD }) => {
	...
	return (
		<Fragment>
			{data &&
				data.map((d, j) => (
					<Grid container key={j} spacing={3} className={classes.bCard}>
						<Grid item xs={12} sm={4}>
							<Link key={j} to={`/blog/${d.link}`}>
								<img alt={d.title} className={classes.blogImg} src={d.img} />
							</Link>
						</Grid>
...
const FilterCard = ({ setCatg, setPage }) => {
	const classes = useStyles();
	const tags = tag.data.read();
	const categ = blogCat.data.read();
	const authors = author.data.read();

	return (
		<Paper className={classes.paper} style={{ position: "sticky", top: 50 }} elevation={4}>
			...
			<List dense>
				{categ.map((c) => (
					...
					>
						<ListItemText primary={`»${"\u00A0"} ${c.catgName}`} className={classes.listText} />
					</ListItem>
				))}
			</List>
			<Divider />
			...
			<List dense>
				{authors.map((c) => (
					<ListItem
						...
						className={classes.list}
					>
						<ListItemText primary={`»${"\u00A0"} ${c.author}`} className={classes.listText} />
					</ListItem>
				))}
			</List>
			<Divider />
